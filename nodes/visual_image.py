...
class PromptStorage:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                "文件路径": ("STRING", {"default": media_dir, "multiline": False}),
                "媒体文件": (media_files, ),
            },
            "optional": {
                "选中的正向提示词": ("STRING", {"default": "", "multiline": False}),
                "选中的负向提示词": ("STRING", {"default": "", "multiline": False}),
            },
            "hidden": {
                "unique_id": "UNIQUE_ID",
            }
    ...
    FUNCTION = "load_media"
    CATEGORY = "prompt"
    DESCRIPTION = "从 output 目录加载图片,输出提示词信息"

    def load_media(self, 文件路径, 媒体文件, 选中的正向提示词="", 选中的负向提示词="", unique_id=None):
        # 打印接收到的参数
        print(f"[load_media] received selected_positive_prompt: '{选中的正向提示词}'")
        print(f"[load_media] received selected_negative_prompt: '{选中的负向提示词}'")
        ...
        # 注意：选中的正向提示词 和 选中的负向提示词 是由前端 widget 传递的，
        # 如果用户从未选中或取消选中，它们为空字符串，此时应输出空字符串，不再回退到保存的所有提示词。
        if 选中的正向提示词 and 选中的正向提示词.strip():
            prompt_positive = 选中的正向提示词.strip()
        ...
        if 选中的负向提示词 and 选中的负向提示词.strip():
            prompt_negative = 选中的负向提示词.strip()
