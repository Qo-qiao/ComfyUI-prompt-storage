...
class TextModifier:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "输入(Input)": ("STRING", {"forceInput": True}),
    ...
    def modify_text(self, **kwargs):
        输入_Input = kwargs.get('输入(Input)', kwargs.get('输入_Input', ''))
        ...
        print(f"[TextModifier] received input: '{输入_Input}'")
        ...
        if 模式_Mode == "替换(Replace)":
            if 文本内容_Text == "":
                result = 整段文本_FullText
            else:
                result = 整段文本_FullText.replace(文本内容_Text, 输入_Input)
        elif 模式_Mode == "追加文本前(Append Before)":
            if 文本内容_Text == "":
                result = 输入_Input + 整段文本_FullText
            else:
                idx = 整段文本_FullText.find(文本内容_Text)
                if idx != -1:
                    result = 整段文本_FullText[:idx] + 输入_Input + 整段文本_FullText[idx:]
                else:
                    result = 输入_Input + 整段文本_FullText
        else:
            if 文本内容_Text == "":
                result = 整段文本_FullText + 输入_Input
            else:
                idx = 整段文本_FullText.find(文本内容_Text)
                if idx != -1:
                    result = 整段文本_FullText[:idx + len(文本内容_Text)] + 输入_Input + 整段文本_FullText[idx + len(文本内容_Text):]
                else:
                    result = 整段文本_FullText + 输入_Input
